...
benchmarks/python.csv: perf.py
	for t in 1 2 3 4 5; do python3.7 $<; done >$@

benchmarks/python2.csv: perf_python.py
	for t in 1 2 3 4 5; do python3.7 $<; done >$@

benchmarks/numba.csv: perf_numba.py
	for t in 1 2 3 4 5; do $(HOME)/miniconda3/bin/python3 $<; done >$@

benchmarks/pypy.csv: perf_pypy.py
	for t in 1 2 3 4 5; do pypy3 $<; done >$@

benchmarks/cython.csv: perf_cython.py
	for t in 1 2 3 4 5; do python3.7 $<; done >$@

benchmarks/pythran.csv: perf_pythran.py
	for t in 1 2 3 4 5; do python3.7 $<; done >$@

benchmarks/python_select.csv: perf_python_select.py
	for t in 1 2 3 4 5; do $(HOME)/miniconda3/bin/python3 $<; done >$@
...
LANGUAGES = c fortran java javascript julia python python2 pypy numba cython pythran python_select
